<div>
    <div class="d-flex align-items-center justify-content-between mb-4 pb-2">
        <h1 class="h2 mb-0"></h1><a class=" mt-3 fw-bold text-decoration-none" href="#"><i class="fi-x fs-xs mt-n1 me-2"></i> حذف همه</a>
    </div>

    @foreach($products as $index=> $product)
        <div class="card card-hover card-horizontal border-0 shadow-sm mb-4">
            <div class="card-img-top position-relative" style="background-image: url({{ asset($product->images()->first()->file_path) }});"><a wire:navigate class="stretched-link"
           href="{{ route('singleProduct',['id'=>$product->id,'slug'=>slugMaker($product->name)]) }}"></a>
                <div class="position-absolute start-0 top-0 pt-3 ps-3"><span class="d-table badge bg-success mb-1">فعال</span><span class="d-table badge bg-info">جدید</span>
                </div>
