    ...
    @if($products->isNotEmpty())
        ...
        @foreach($products as $index=> $product)
            <div wire:key="{{ $product->id }}" class="card card-hover card-horizontal border-0 shadow-sm mb-4">
                <div class="card-img-top position-relative" style="background-image: url({{ asset($product->images()->first()->file_path) }});"><a wire:navigate class="stretched-link" href="{{ route('singleProduct',['id'=>$product->id,'slug'=>slugMaker($product->name)]) }}"></a>
                    <div class="position-absolute start-0 top-0 pt-3 ps-3">
                        <span class="d-table badge bg-success mb-1">فعال</span>
                        <span class="d-table badge bg-info">جدید</span>
                    </div>
                    <div class="position-absolute end-0 top-0 pt-3 pe-3 zindex-5">
                        <button class="btn btn-icon btn-light btn-xs text-primary rounded-circle shadow-sm" type="button" data-bs-toggle="tooltip" data-bs-placement="right" aria-label="حذف از مورد علاقه" data-bs-original-title="حذف از مورد علاقه"><i class="fi-heart-filled"></i>
                        </button>
                    </div>
                </div>
                ...
                ($product->name)]) }}">{{ $product->name }}</a></h3>
                    ...
                    <div class="d-flex align-items-center justify-content-center justify-content-sm-start border-top pt-3 pb-2 mt-3 text-nowrap">
                        <span class="d-inline-block me-4 fs-sm">3<i class="fi-bed ms-1 mt-n1 fs-lg text-muted"></i></span><span class="d-inline-block me-4 fs-sm">2<i class="fi-bath ms-1 mt-n1 fs-lg text-muted"></i></span><span class="d-inline-block fs-sm">2<i
