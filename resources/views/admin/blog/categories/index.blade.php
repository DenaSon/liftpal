...
@section('page-title','دسته بندی های مقالات')
@section('CustomCss')
...
@foreach($categories as $index => $category)
    <li class="list-group-item mt-1 border-1">
        <div class="d-flex justify-content-between align-items-center">
            <b class="">

                {{ $category->name }}  </b>
            <form id="deleteForm-{{$category->id}}" action="{{ route('blogCategories.destroy', ['blogCategory' => $category->id]) }}" method="POST">
                @csrf
                @method('delete')
                ...
                <a class="btn btn-sm btn-outline-secondary" href="{{ route('blogCategories.edit',['blogCategory' => $category->id]) }}" >
...
@section('CustomJs')
    ...
    @include('admin.blog.categories.inc._scripts')
