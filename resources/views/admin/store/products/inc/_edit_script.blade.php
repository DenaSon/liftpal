...
<script>
    ...
    $(document).ready(function () {
        ...
        categorySelect.hover(function () {
            ...
            const selectedCategories = {!! json_encode($product->categories->pluck('id')) !!};
...
<script>
    ...
    $(".edit-type").on("click", function(){
        ...
        var name = $("#row-" + typeId + " input[name='optionname']").val();
        var price = $("#row-" + typeId + " input[name='optionprice']").val();
        ...
        $.ajax({
            ...
            data: {
                newName: name,
                newPrice: price,
                id: typeId // Pass the typeId as the 'id' parameter
            },
            success: function(response){
                const msg = ' گزینه ' + name  + ' با قیمت '+ price + ' ثبت شد ';
                Swal.fire({
                    title: msg,
                    icon: 'success',
                    position: 'center', // Position of the toast on the screen
                    showConfirmButton: false, // Hides the "OK" button
                    timer: 1200 // Duration in milliseconds (e.g., 3000ms = 3 seconds)
                });
            },
            error: function(xhr, status, error) {
                Swal.fire({
...
<script>
    ...
    $(".remove-type").on("click", function(){
        ...
        Swal.fire({
            ...
            text: 'این گزینه با اطلاعات انبار آن حذف خواهد شد!',
            icon: 'question',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    success: function(response){
                        const row = $("#row-" + typeId);
                        row.css("background-color", "#ea7979").hide(150);
                        //row.hide(100);
